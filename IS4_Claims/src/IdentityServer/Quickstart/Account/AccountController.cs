...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ListUsersRoles()
        {
            ...
            var roles = _roleManager.Roles.ToList();
            var vm = new List<DisplayUserRoleViewModel>();
            ...
            foreach (var u in user)
            {
                foreach(var r in roles)
                {
                    ...
                    {
                        vm.Add(new DisplayUserRoleViewModel
                        {
                            Username = u.UserName,
                            Rolename = r.Name
                        });
        ...
        [HttpGet]
        [Authorize(Roles = "hr,manager,admin,business")]
        public async Task<IActionResult> ListClaimUsers(string userId)
        {
            var user=await _userManager.FindByIdAsync(userId);
            if (user == null)
            {
            ...
            var claims = await _userManager.GetClaimsAsync(user);
            var vm = new DisplayUserClaimViewModel();
            vm.Username = user.UserName;
            if (claims != null)
            {
                foreach (var c in claims)
                {
                    vm.Claims.Add(c.Type);
                }
            }
            return View(vm);
        }
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // validate username/password against DB store
                if (result.Succeeded)
                {
